...
/** Integração externa: substituir comentário por WhatsApp. */
async function updateLeadComentarioIntegrationByWhatsapp(whatsappNumberRaw, comentarioRaw) {
...
async function createLeadWeb(dados) {
  ...
  const nome = (dados.nome || '').trim() || null;
  const email = (dados.email || '').trim() || null;
  if (!nome || !email || !whatsapp_number) return null;
  ...
  if (next && next.id) {
    gestora_id = next.id;
    if (next.nome) gestora_nome = String(next.nome).trim() || null;
  }
  ...
  const rows = await query(
    'SELECT id, whatsapp_number, nome, email, estado_conversa, estado_docs, gestora_id, gestora_nome, created_at, updated_at FROM ch_leads WHERE id = LAST_INSERT_ID()'
  );
  return rows[0] || null;
}
...
async function getGestorasWithLeadCounts() {
  const rows = await query(
    `SELECT g.id, g.nome, g.email, g.email_para_leads, g.whatsapp, g.ativo, g.created_at, g.updated_at,
       ...
       SUM(CASE WHEN l.estado_docs = 'pre_aprovado' THEN 1 ELSE 0 END) AS pre_aprovado,
  ...
  return rows.map((r) => ({
    ...
    pre_aprovado: Number(r.pre_aprovado) || 0,
